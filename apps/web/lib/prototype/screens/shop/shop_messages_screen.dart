...
/// Filtered chat inbox showing only shop/marketplace conversations
class ShopMessagesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final shopConvos = ProtoDemoData.conversations
        .where((c) => c.moduleContext == 'Market')
        .toList();
      ...
      body: Column(
        ...
        children: [
          Padding(
            ...
            child: Text('Messages', style: theme.headline.copyWith(fontSize: 24)),
          ),
          Padding(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    ...
                    Text('Search messages...', style: theme.body.copyWith(color: theme.textTertiary)),
                  ],
                ),
              ),
            ),
          ...
          Expanded(
            child: shopConvos.isEmpty
                ? Center(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(theme.icons.forum, size: 48, color: theme.textTertiary),
                        const SizedBox(height: 12),
                        Text('No marketplace messages yet', style: theme.body.copyWith(color: theme.textSecondary)),
                        const SizedBox(height: 4),
                        Text('Start buying or selling to chat with others', style: theme.caption),
                      ],
                    ),
                  )
                : ListView.builder(
                    padding: const EdgeInsets.symmetric(horizontal: 16),
                    itemCount: shopConvos.length,
                    itemBuilder: (context, i) {
                      final conv = shopConvos[i];
